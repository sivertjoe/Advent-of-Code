...
(defun execute (input commands)
  (let* ((script (string-to-chars (build-command commands)))
    ...
    (loop
      for res = (run comp)
      while (/= (car res) *exit*)
      finally (return (car (output comp))))))

(defun part-one (input)
  (execute input (list "NOT C T" 
                       "OR T J"  ;; Jump if C is empty
                       "AND D J" ;; and D is NOT empty
                       "NOT A T" 
                       "OR T J" ;; OR, if A is empty
                       "WALK")))

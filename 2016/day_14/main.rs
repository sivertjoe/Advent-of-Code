...
fn search<const N: usize, F>(salt: &str, f: F) -> usize
where
    F: Fn(&str) -> md5::Digest,
{
    let mut res = Vec::new();
    let mut threes: HashMap<u8, Vec<usize>> = HashMap::new();
    let mut fives: HashMap<u8, HashSet<usize>> = HashMap::new();

    for i in 0..N
    {
        let hash = f(&format!("{}{}", salt, i));
        let hash = format!("{:x}", hash);

        let mut flag = true;
        for arr in hash.as_bytes().windows(5)
        {
            let v = arr[0];
            if arr.iter().all(|elem| *elem == v)
            {
                fives.entry(v).or_default().insert(i);
            }

            if flag
            {
                let cmp = |i| (arr[i] == arr[i + 1] && arr[i] == arr[i + 2]);
                let v = if cmp(0)
                {
                    Some(arr[0])
                }
                ...
                {
                    flag = false;
                    threes.entry(v).or_default().push(i);
                }
            }
        }
    }

    for (val, threes) in threes
    {
        for three in threes
        {
            if let Some(fives) = fives.get(&val)
            {
                for five in fives
                {
                    if *five > three && *five - three <= 1000
                    {
                        res.push(three);
                    }
                }
    ...
    res.sort();
    res.into_iter().nth(63).unwrap()
}

fn strech(s: &str) -> md5::Digest
{
...
{
    let salt = &input[0];
    let r#fn = |s: &str| md5::compute(s);
    search::<30_000, _>(salt, r#fn)
}

fn task_two(input: &[String]) -> usize
{
    let salt = &input[0];
    search::<25_000, _>(salt, strech)
}
...
mod md5
{
    // Copied directly from the md5 crate
    #[cfg(feature = "std")] use core::io;
    use std as core;

    use self::core::{convert, fmt, ops};

...
fn neighbors(passcode: &str, path: &str, (x, y): (usize, usize)) -> Vec<(usize, usize)>
{
    ...
    vec.into_iter()
        .filter(|(nx, ny)| *nx <= 3 && *ny <= 3 && (*nx, *ny) != (x, y))
        .collect()
}

fn letter(pos: (usize, usize), new: (usize, usize)) -> char
{
    if new.0 < pos.0
    {
        'L'
...
fn solve(input: &[String], early_exit: bool) -> String
{
    ...
    let mut vec: VecDeque<((usize, usize), String)> = VecDeque::new();
    vec.push_back((pos, "".into()));

    let mut max = String::new();
    while let Some((pos, path)) = vec.pop_front()
    {
        if pos == (3, 3)
        {
            max = std::cmp::max_by_key(max, path, |s| s.len());
            if early_exit
            {
                break;
            }
        ...
        for neighbor in neighbors(passcode, &path, pos)
        {
            let letter = letter(pos, neighbor);
            let mut path = path.clone();
            path.push(letter);
            if seen.insert(path.clone())
            {
                vec.push_back((neighbor, path));
            }
        }
    }
    max
